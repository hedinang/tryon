...
from data.base import BodyPoseModel


model_url = 'https://www.dropbox.com/s/mun9eh2509pw32n/openpose_body_coco_pose_iter_440000.pth?dl=1'
model_dir = 'checkpoints/openpose_body_coco_pose_iter_440000.pth'
...
def _get_keypoints(candidates, subsets):
    ...
    ss = np.zeros((18, 3), dtype=np.int32)
    for i in range(k):
        for j in range(18):
...
class BodyPoseEstimator:
    ...
    def __init__(self, model_dir):
        ...
        self._model = _load_state_dict(self._model, torch.load(model_dir))
        self._model.eval()
    ...
    def __call__(self, image):
        ...
        for scale in multipliers:
            ...
            image_tensor = np.expand_dims(
                np.transpose(image_padded, (2, 0, 1)), 0)
            ...
            paf = paf[:image_padded.shape[0] - pads[3],
                      :image_padded.shape[1] - pads[2], :]
            paf = cv2.resize(
                paf, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_CUBIC)
        ...
        for part in range(18):
            ...
            peaks = list(zip(np.nonzero(peaks_binary)[
                         1], np.nonzero(peaks_binary)[0]))
            ...
            if n_A != 0 and n_B != 0:
                ...
                for i in range(n_A):
                    for j in range(n_B):
                        ...
                        vx = np.array([score_mid[int(round(ab[x][1])), int(
                            round(ab[x][0])), 0] for x in range(len(ab))])
                        vy = np.array([score_mid[int(round(ab[x][1])), int(
                            round(ab[x][0])), 1] for x in range(len(ab))])
                        score_midpoints = np.multiply(
                            vx, v[0]) + np.multiply(vy, v[1])
                        score_with_dist_prior = sum(
                            score_midpoints) / len(score_midpoints) + min(0.5 * image.shape[0] / n - 1, 0)
                        criterion_1 = len(np.nonzero(score_midpoints > thresh_2)[
                                          0]) > 0.8 * len(score_midpoints)
                        criterion_2 = score_with_dist_prior > 0
                        if criterion_1 and criterion_2:
                            connection_candidate = [
                                i, j, score_with_dist_prior, score_with_dist_prior + candidate_A[i][2] + candidate_B[j][2]]
                ...
                for candidate in connection_candidates:
                    ...
                    if i not in connection[:, 3] and j not in connection[:, 4]:
                        connection = np.vstack(
                            [connection, [candidate_A[i][3], candidate_B[j][3], s, i, j]])
                ...
                for i in range(len(all_connections[k])):
                    ...
                    if found == 1:
                        ...
                        if subset[j][index_B] != part_Bs[i]:
                            ...
                            subset[j][-1] += 1
                            subset[j][-2] += candidate[part_Bs[i].astype(
                                int), 2] + all_connections[k][i][2]
                    elif found == 2:
                        j1, j2 = subset_idx
                        ...
                        if len(np.nonzero(membership == 2)[0]) == 0:
                            ...
                            subset[j1][-1] += 1
                            subset[j1][-2] += candidate[part_Bs[i].astype(
                                int), 2] + all_connections[k][i][2]
                        ...
                        row[-1] = 2
                        row[-2] = sum(candidate[all_connections[k][i,
                                                                   :2].astype(int), 2]) + all_connections[k][i][2]
