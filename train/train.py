...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):  # 200 epoch
    epoch_start_time = time.time()
    if epoch != start_epoch:
        epoch_iter = epoch_iter % dataset_size

    for i, data in enumerate(dataloader, start=epoch_iter):  # load batch size data
        ...
        # data['edge'] là ảnh grey cloth trong thư mục train edge
        # img_fore là ảnh của người loại bỏ đi background
        # mask_clothes là ảnh random cloth trong thư muc train cloth mask
        # data['color'] là áo rgb thử
        # all_clothes_label là MF nối 2 bàn tay với vùng chứa áo
        # data['image'] là ảnh ground truth
        # data['pose'] là json pose là 18 ảnh có cùng kích thước với ảnh gốc mỗi ảnh được vẽ từ 1 hình vuông
